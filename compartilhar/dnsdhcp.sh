...
USERUPDATE="jonatas"
DOMAIN="nm.local"
DOMAINREV="253.168.192.in-addr.arpa"
NETWORK="192.168.253."
#
...
echo -e "Atualizando os arquivos de configuração do Bind9 DNS Server, aguarde..."
	...
	cp -v conf/192.168.253.rev /var/lib/bind/192.168.253.rev &>> $LOG
...
echo -e "Editando o arquivo 172.16.1.rev, pressione <Enter> para continuar."
	...
	# opção do comando chown: -v (verbose), -root (user), bind (group)
	read
	vim /var/lib/bind/192.168.253.rev
	chown -v root:bind /var/lib/bind/192.168.253.rev &>> $LOG
	named-checkzone $DOMAINREV /var/lib/bind/192.168.253.rev &>> $LOG
	named-checkzone $NETWORK /var/lib/bind/192.168.253.rev &>> $LOG
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
sleep 5
#
